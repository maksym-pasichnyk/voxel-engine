...
- `plan` owns implementation planning quality, phased sequencing, dependency mapping, and pre-implementation verification strategy. It does not own production implementation or architecture authority decisions.
...
- All final agent outputs must be persisted in-repo under `persistant/` for durable access.
- Required location format: `persistant/<agent-name>/<yyyy-mm-dd>/<slug>.md`.
- Every persisted artifact must include:
	...
	- recommended next role
	- timestamp (UTC)
- `persistant/index.md` must be updated with a link entry for each new artifact.
- If an output is revised, append a new versioned artifact instead of overwriting historical records.
...
- User-invocable agents are: `plan`, `core`, `rendering`, `gameplay`, and `proof-of-concept`.
