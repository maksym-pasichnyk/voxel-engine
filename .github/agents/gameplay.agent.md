...
## Input Contract
- Required: player-facing goal, expected outcomes, and failure-state expectations.
- Preferred: plan/architecture constraints and core/rendering dependencies.
- If rules are ambiguous: request clarification before implementing behavior.

## Output Contract
- Deliver explicit gameplay rules, failure handling, and dependency notes.
- Keep gameplay logic separate from engine ownership domains.
- Provide validation/test ideas for user-visible behavior.
